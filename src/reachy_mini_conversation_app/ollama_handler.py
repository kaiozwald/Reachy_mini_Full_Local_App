"""Ollama-based conversation handler with local STT (faster-whisper) and TTS (pyttsx3).

Replaces the OpenAI Realtime API handler with a fully local/self-hosted pipeline:
...
class OllamaHandler(AsyncStreamHandler):
    """Conversation handler using Ollama (LLM), faster-whisper (STT), and pyttsx3 (TTS)."""
    ...
    def __init__(
        ...
        self._frame_duration: float = 0.0  # set on first receive()

        # TTS lock – prevents overlapping synthesis
        self._tts_lock: asyncio.Lock = asyncio.Lock()

        # Timing
        self.last_activity_time: float = 0.0  # set after event loop is running
    ...
    async def start_up(self) -> None:
        ...
        try:
            ...
            logger.error("Cannot reach Ollama at %s: %s", ollama_host, e)
        ...
        logger.info(
            "OllamaHandler ready — model=%s  stt=%s  tts_voice=%r",
        ...
        if rms > SILENCE_RMS_THRESHOLD:
            ...
            if self._is_speaking:
                ...
                if silence_duration >= SILENCE_DURATION_S:
                    ...
                    # Reset state BEFORE launching task (avoid double triggers)
                    self._audio_buffer = []
    ...
    async def _chat_with_tools(self) -> str:
        """Send conversation to Ollama; handle tool calls; return final text."""
        if self.ollama_client is None:
            return "Ollama client not initialized."

        model_name = getattr(config, "MODEL_NAME", "llama3")
        ...
        for _iteration in range(10):
            ...
            assistant_msg: dict[str, Any] = response.get("message", {})
            tool_calls: list[dict] = assistant_msg.get("tool_calls") or []

            if not tool_calls:
                # Final text response
                response_text: str = assistant_msg.get("content", "")
                if response_text:
                    self._messages.append({"role": "assistant", "content": response_text})
                return response_text

            # ---- process tool calls ----
            # Append the assistant message (contains tool_calls) so history is valid
            ...
            for tc in tool_calls:
                ...
                # Append tool result to conversation so Ollama can continue
    ...
    async def _synthesize_speech(self, text: str) -> None:
        ...
        async with self._tts_lock:
            ...
            try:
                def _tts_save() -> bool:
                    """Run pyttsx3 synchronously and save to tmp_path. Returns True on success."""
                    try:
                        engine = pyttsx3.init()
                ...
                # Decode WAV → int16 PCM at HANDLER_SAMPLE_RATE
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("OllamaHandler shut down.")
